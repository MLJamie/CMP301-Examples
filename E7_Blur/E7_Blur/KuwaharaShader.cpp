// kuwahara oil painting shader
#include "KuwaharaShader.h"


KuwaharaShader::KuwaharaShader(ID3D11Device* device, HWND hwnd) : BaseShader(device, hwnd)
{
	initShader(L"kuwahara_vs.cso", L"kuwahara_ps.cso");
}
...
void KuwaharaShader::initShader(const wchar_t* vsFilename, const wchar_t* psFilename)
{
...
void KuwaharaShader::setShaderParameters(ID3D11DeviceContext* deviceContext, const XMMATRIX& worldMatrix, const XMMATRIX& viewMatrix, const XMMATRIX& projectionMatrix, ID3D11ShaderResourceView* texture, float width, float height)
{
	...
	screenPtr->padding = XMFLOAT2(1.0f, 1.0f);
